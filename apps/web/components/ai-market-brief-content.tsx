...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h4 className="text-[10px] font-bold uppercase tracking-[0.18em] text-accent">{children}</h4>
  );
}

/** Narrow optional UI hook: reveals the grounded evidence for a claim without exposing IDs in prose. */
function ShowEvidenceButton({
  refs,
  onShowEvidence,
}: {
  refs: string[];
  ...
  if (!onShowEvidence || refs.length === 0) return null;
  return (
    <button
      type="button"
      onClick={() => onShowEvidence(refs)}
      className="mt-2 inline-flex items-center gap-1 rounded-full bg-white/80 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-accent ring-1 ring-line transition-colors hover:bg-sakura-100 hover:text-brand-deep"
    >
      Show evidence
    </button>
  );
...
export function AiMarketBriefContent({
  brief,
  onShowEvidence,
}: {
  ...
  return (
    <>
      ...
      <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-ink-secondary">
        <span className="rounded-full bg-ink/5 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider text-ink-muted">
          {brief.stance.label}
        </span>
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        {brief.overview.map((item, i) => (
          <p key={`overview-${i}`} className="text-sm leading-relaxed text-ink-secondary">
            {item.text}
          </p>
        ))}
      </div>
            ...
            {brief.keyDrivers.map((driver, i) => (
              <li key={`driver-${i}`} className="rounded-xl border border-line bg-sakura-50/70 p-3">
                <p className="flex items-center gap-2 text-sm font-semibold text-ink">
                  <span
                    aria-hidden
                    className={`rounded px-1.5 py-0.5 text-[10px] ${impactTone[driver.impact] ?? "bg-ink/10 text-ink-muted"}`}
                  >
                    {impactGlyph[driver.impact] ?? "•"}
                  </span>
                  <span className="sr-only">{driver.impact}</span>
                  {driver.title}
                </p>
                <p className="mt-1 text-xs leading-relaxed text-ink-secondary">{driver.text}</p>
                <ShowEvidenceButton refs={driver.evidenceRefs} onShowEvidence={onShowEvidence} />
              </li>
            ))}
          </ul>
        </section>
          ...
          <ul className="mt-2 flex flex-col gap-2">
            {brief.notableMoves.map((move, i) => (
              <li key={`move-${i}`} className="rounded-xl border border-line bg-white/60 p-3">
                ...
                <ShowEvidenceButton refs={move.evidenceRefs} onShowEvidence={onShowEvidence} />
              </li>
            ))}
          </ul>
        </section>
      ...
      <footer className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-line pt-3 text-[11px] text-ink-muted">
        <span className="font-semibold uppercase tracking-wider">
          {brief.dataQuality.confidence} confidence
        </span>
