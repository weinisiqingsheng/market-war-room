...
export function useChineseAiMarketBrief() {
  ...
  const load = useCallback(async () => {
    ...
    catch { if (id === requestId.current && !controller.signal.aborted) setStatus("error"); }
    finally { if (activeController.current === controller) activeController.current = null; }
  }, []);
  useEffect(() => {
    let disposed = false;
    queueMicrotask(() => { if (!disposed) void load(); });
    const timer = setInterval(() => { if (!document.hidden) void load(); }, 180_000);
    return () => { disposed = true; activeController.current?.abort(); clearInterval(timer); };
  }, [load]);
