...
export interface ChineseAskApiSuccess {
  mode: "demo" | "live";
  status: "generated";
...
export type ChineseAskUiStatus =
  "idle" | "submitting" | "success" | "api_insufficient" | "unavailable";
export interface ChineseAskResult {
  status: ChineseAskUiStatus;
  data: ChineseAskApiSuccess | null;
...
export function useChineseAskWarRoom(): ChineseAskResult {
  ...
  const run = useCallback(async (raw: string) => {
    ...
    try {
      const response = await fetch("/api/zh/ai/ask-war-room", {
        method: "POST",
      ...
      if (!response.ok) {
        const errorCode =
          typeof (json as { error?: { code?: unknown } }).error?.code === "string"
            ...
            : null;
        setErrorKind(
          errorCode === "invalid_request" || errorCode === "too_short" || errorCode === "too_long"
            ? "invalid_request"
            : "unavailable",
        );
        setStatus("unavailable");
      ...
      if (json.status === "generated" && json.answer) {
        setData({
          mode: json.mode ?? "demo",
          status: "generated",
          contextFingerprint: json.contextFingerprint,
          inputConfidence: json.inputConfidence,
      ...
      if (!disposed.current && id.current === current) {
        setErrorKind("unavailable");
        setStatus("unavailable");
      }
    }
